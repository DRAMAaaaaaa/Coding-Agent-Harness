...
- Provider 管理、持久凭据 UI、probe/smoke：当前替代为会话凭据和离线 Mock；影响是真实 Provider 未验收；完成凭据生命周期与网络治理后重启。
- 通用/加密检查点、多级纠正分支、复杂学习卡：当前替代为单级纠正和每项目一条经验；影响是不能表达复杂历史；完成版本化、加密与确定性回归后重启。
- 最终视觉设计：当前替代为最小表单与引用展示；影响仅为可用性；主路径 E2E 稳定后重启。
