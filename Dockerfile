...
RUN mkdir -p /state && chown -R harness:harness /app /state
...
CMD ["python", "scripts/serve_demo.py", "--ready-file", "/state/ready.json", "--runtime-root", "/state", "--host", "0.0.0.0", "--port", "8000", "--max-seconds", "86400"]
