...
| MVP-ISSUE-016 | Important | `scripts/test.ps1` 在隔离 worktree 缺少 `.venv` 时，`Resolve-Path` 失败后仍继续执行，并可能由后续成功命令覆盖退出码形成假阳性；缺少 `web/node_modules` 时 Web 门禁也无法启动 | 待 MVP-4 Task 9 修复 | 不阻塞已完成的 MVP-1 安全项目接入。Task 9 必须让一键入口对缺失前置条件 fail closed，并在隔离 worktree/CI 中验证真实 Python 与 Web 门禁；当前主控通过锁文件哈希一致的本地 junction 复用已批准依赖，并以固定解释器取得真实门禁证据 |
