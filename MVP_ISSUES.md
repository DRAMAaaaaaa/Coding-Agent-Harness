...
| MVP-ISSUE-003 | Important | `pyproject.toml`/`package.json` 推导命令不要求信任，配置变化无法使旧审批失效 | 关闭 | MVP Task 4 已实现 `verification-trust/v1`，绑定三源 raw 摘要与规范化有效配置；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-004 | Important | Python/Node fixture 只断言 argv，不能证明建议验证命令在独立目录中可运行 | 关闭 | MVP Task 4 已修复 Python src package，并让 Node 24 四项命令离线自包含；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
...
| MVP-ISSUE-006 | Minor | 10,000 文件性能测试在 CI 中硬断言 5 秒，与计划的“慢机只记录”冲突 | 关闭 | MVP Task 4 已让 CI 只验证功能并记录耗时，本机独立基准仍验收小于 5 秒；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
...
| MVP-ISSUE-008 | Important | Detector 在拒绝三类符号链接/reparse 配置前先执行 follow-target `exists`，底层 opener 也在无 no-follow 保护下打开目标 | 关闭 | 三入口已先以 `lstat`/reparse 属性分类，记录型 opener 为零调用；默认 opener 在打开前拒绝静态链接并在打开后复验句柄身份。最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-009 | Minor | `PLAN.md` 总表与 Task 5 报告把历史 2 Critical、2 Important、3 Minor 误写成当前状态 | 关闭 | 当前状态已统一；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-010 | Critical | 不同 Windows drive/UNC anchor 及含 drive/root 但非 absolute 的路径在固定拒绝前仍进入 `resolve`/`samefile`，普通工具与 worktree 注册消费可触发未批准探测 | 关闭 | 普通 containment 已在相对拼接前使用纯词法 anchor/不明确路径门禁并保持零探测；技术修复已纳入 `af11fea`。最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-011 | Critical | SafeGit 一般命令与原 `--local` 审计仍依赖 repository discovery，可由 path/include、primary/linked `commondir` 或 `worktreeConfig` 重定向并读取仓外元数据 | 关闭 | primary 已拒绝 `commondir`，linked 每命令复验严格 `../..`；已拒绝 `extensions.worktreeConfig`/`config.worktree`，并对已验证 common config 使用无 `-C` 的 `--file ... --no-includes` 审计。技术修复已纳入 `af11fea`；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-012 | Important | `git-safety`/`hooks` 静态 junction/reparse 可让 SafeGit 构造阶段在 `state_root` 外创建文件 | 关闭 | safety 目录与空文件已在创建前后使用 no-follow reparse 分类、containment 和句柄/路径身份复验；同 UID 主动竞态仍仅由 `DW-05-001` 排除。技术修复已纳入 `af11fea`；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-013 | Important | create 最终 `same_path` 身份异常泄漏 `UnsafePathNamespaceError`，没有统一进入不确定副作用契约 | 关闭 | 最终条件已纳入同一异常映射边界并完整回归 `.active` 保留；技术修复已纳入 `af11fea`；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-014 | Important | scanner 对 tracked symlink/reparse 在 no-follow 分类前调用 `exists()`，可在拒绝前 follow 外部或 UNC 目标 | 关闭 | candidate 已先 `lstat` 并拒绝 symlink/reparse；消费者级记录型测试证明 follow-target `exists/stat/resolve` 均为零。技术修复已纳入 `af11fea`；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-015 | Important | scanner 对 `dir/file` 仅先检查 leaf；若既有父目录是 symlink、junction 或 reparse，则 leaf `lstat`/`resolve` 已在拒绝前穿越父目录并访问根外对象 | 关闭 | 已从已解析 root 沿 `PurePosixPath.parts` 对每个既有非叶组件执行 no-follow `lstat`，仅允许普通非链接/非 reparse 目录；安全父下才检查 leaf。技术修复为 `c376216`；最终双门禁均为 CLEAN，见 `.superpowers/sdd/mvp1-post-parent-spec-clean.md` 与 `.superpowers/sdd/mvp1-post-parent-quality-clean.md` |
| MVP-ISSUE-016 | Important | `scripts/test.ps1` 在隔离 worktree 缺少 `.venv` 时，`Resolve-Path` 失败后仍继续执行，并可能由后续成功命令覆盖退出码形成假阳性；缺少 `web/node_modules` 时 Web 门禁也无法启动 | 已关闭（MVP-4 Task 9） | PowerShell 与跨平台 Makefile 均在执行任何测试前固定检查 Python、npm 和 `web/node_modules`，缺失时输出固定、可操作、无秘密的诊断并立即以 2 退出；回归分别在无 `.venv` 与只有占位解释器、无 Web 依赖的隔离目录得到非零结果。根 pytest 配置固定当前 `src` 与正式 `tests`，避免 editable 主仓包和故意失败的 demo fixture 污染一键结果；Windows `mingw32-make demo/test-e2e` 已取得真实入口证据。 |
