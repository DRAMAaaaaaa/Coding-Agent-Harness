...
| MVP-ISSUE-015 | Important | scanner 对 `dir/file` 仅先检查 leaf；若既有父目录是 symlink、junction 或 reparse，则 leaf `lstat`/`resolve` 已在拒绝前穿越父目录并访问根外对象 | 技术修复已提交（`c376216`），等待最终规约与质量双门禁 | 已从已解析 root 沿 `PurePosixPath.parts` 对每个既有非叶组件执行 no-follow `lstat`，仅允许普通非链接/非 reparse 目录；安全父下才检查 leaf。未使用布尔 containment 缓存；同 UID 主动竞争仍属于 `DW-05-001`。门禁未通过前不得关闭 |
