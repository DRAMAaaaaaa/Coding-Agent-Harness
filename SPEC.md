...
首次选择真实 provider 时，WebUI 引导隐藏录入；保存后仅显示状态。用户可以更新和清除。本机使用 OS keyring；容器使用主密码 + Argon2id + AES-256-GCM 认证加密文件与持久卷。`.env.example` 只提供不会被产品读取的占位说明；产品不得把 `.env` 作为凭据回退路径。
...
## 16. 共学回放式 Harness 扩展（2026-08-07 批准）

### 16.1 产品特色

下一阶段在已完成 MVP 上增量构建“证据驱动的共学回放式 Coding Harness”。产品首先帮助程序员理解真实项目中的工程决策：Agent 为什么采取某个动作、依赖了哪些事实、预期什么结果、实际证据是什么，以及验证失败后为什么改变方向。

原始执行轨迹不可改写；用户可以在语义检查点提问、纠正或补充需求。提问是零工具、零副作用的只读解释；纠正从检查点创建新轨迹和独立 worktree；补充需求生成新计划版本并使旧批准失效。回放基本单位是结构化意图卡，不保存或展示模型原始思维链、隐藏提示词、凭据与隐私信息。

用户纠正只能生成项目学习卡候选。候选必须经过敏感过滤，并在用户编辑或批准后才能进入长期记忆。学习卡包含作用域、来源、版本、正反示例和使用记录；后续意图卡必须显示采用了哪张卡及其实际影响。产品默认使用引导执行，在计划批准、首次修改、验证失败、方向变化、危险审批和最终交付等语义检查点暂停，同时保留自动执行模式。

### 16.2 四阶段范围

本轮只实施四个顺序依赖、可独立验收的阶段：

1. 真实 DeepSeek/Qwen Provider、凭据生命周期和任务级 LLM 联网授权。
2. 结构化意图卡、语义检查点、只读提问和引导/自动双模式。
3. 加密检查点、不可篡改原轨迹、纠正分支、重放恢复和轨迹比较。
4. 项目学习卡候选、批准、作用域、版本、确定性检索、冲突和引用追踪。

依赖安装、通用工具网络、Git push/merge/release、外部 checkout filter、多 Agent、最终 UI 设计和正式公网部署不属于本轮，必须另立规格。完整批准设计见 `docs/superpowers/specs/2026-08-07-co-learning-replay-harness-design.md`。

### 16.3 新增强制边界

- DeepSeek 与 Qwen 复用现有 `LLMProvider` 和 OpenAI-compatible adapter；不得增加第二套 Agent 主循环。
- Provider 地址固定为 Harness 登记的 HTTPS 地址，不跟随重定向；任务级 LLM 授权绑定一个确切版本的 ProviderProfile，配置变化会使旧授权失效，且不能扩展为工具网络授权。
- 本机持久凭据必须使用 OS Keyring；不可用时只允许进程内会话凭据。容器使用主密码派生密钥和认证加密文件，禁止明文回退。
- 真实 Provider 核心测试使用 HTTP Stub；真实联网只通过用户显式运行的独立冒烟命令验收，不进入 CI。
- 检查点变化数据使用独立安装数据密钥加密；默认保留 30 天，可固定保留或提前清理。过期后保留只读回放，但不得继续分支。
- 学习卡首版使用有界、可复现的作用域过滤和排序，不引入向量数据库；冲突时进入 `WAITING_USER`。
- 现有 Mock 三机制、`make test`、双 CI、治理、worktree 和确定性反馈闭环不得回退。

### 16.4 旗舰验收

用户提交真实项目需求并查看意图卡，在验证失败节点提出只读问题，再从该节点提交纠正并创建新轨迹。新轨迹修改代码并通过验证；用户比较两条轨迹、批准一张项目学习卡。下一个任务确定性检索该卡，并在意图卡中展示它如何改变了 Agent 决策。该场景必须可由 Mock 完全离线、确定性重复运行。
