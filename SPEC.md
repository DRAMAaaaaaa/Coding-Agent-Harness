...
## 16. 共学回放式 Harness 精简扩展（2026-08-07 修订批准）

### 16.1 产品特色与唯一主路径

本扩展在已完成 MVP 上增加“证据驱动的共学回放”能力。用户能够理解 Agent 在关键节点的意图，在验证失败时进行零工具提问，从失败节点提交纠正并创建新轨迹，最后批准一条项目经验供下一任务确定性引用。

意图卡只展示行动目的、事实依据、动作、预期和实际结果，不保存或展示模型原始思维链、隐藏提示词、凭据或隐私信息。原始任务轨迹只追加，任何纠正都创建独立任务和 worktree。

### 16.2 四阶段最小范围

1. **CL-1：** 会话 API Key、DeepSeek/Qwen 选择和现有 Agent 主路径。
2. **CL-2：** 计划、首次修改、验证失败、最终交付四类意图卡；只在验证失败节点提供零工具只读提问。
...
- 复用现有 `AgentOrchestrator`、`LLMProvider`、事件存储、治理、反馈、工具和 worktree，不增加第二套循环或状态事实来源。
- Provider 地址固定，`trust_env=False`，不跟随重定向；任务级 LLM 授权不能扩展为工具网络授权。
- 首版产品路径只接受进程内会话凭据；已实现的 Keyring/加密 Vault 可保留，但不再扩展其管理 API、WebUI 或完成门禁。
- 只读提问使用任务 Provider 和空工具集合，失败不改变任务状态。
- 检查点只保存有界文本 diff、基准提交、事件序号和摘要；摘要不匹配、过大或恢复失败时进入 `WAITING_USER` 并保留现场。
- 创建纠正分支前必须冻结父 worktree 的写租约但保留其目录、分支和内容；同一 Workspace 始终最多一个活动 writer。
- 每个验证失败节点最多一个纠正分支；重复请求必须幂等返回已有分支。
- 经验只有在用户编辑并批准、通过敏感过滤后才能进入下一任务；首版只加载同一项目最近一条。
- 现有 Mock 三机制、`make test`、双 CI、安全护栏、worktree 和确定性反馈闭环不得回退。

### 16.4 明确延期

Provider 管理、持久凭据 UI、Vault 解锁、probe、独立真实 smoke、通用检查点、加密/自动清理、任意恢复、多级分支、复杂比较、学习卡版本/冲突/负例/向量检索、双执行模式和最终 UI 设计全部延期，不属于首版完成标准。

### 16.5 旗舰验收

离线 Mock 场景必须确定性复现：任务验证失败并生成意图卡；用户提出只读问题；用户从失败节点创建纠正分支；新分支通过验证；用户比较结果并批准一条经验；下一任务引用该经验且 Mock Agent 因此改变动作。真实联网未由用户显式提供凭据并触发时，只能记录“未执行”。

### 16.6 当前工作台与交付导航（2026-08-14）

当前产品入口是六阶段共学工作台：项目接入、需求描述、计划审批、执行与验证、回放与纠正、交付与经验。它把既有共学路径投影为可回看的用户界面，不新增第二个 Agent 循环、状态来源或网络能力。

当前可交付能力、证据与限制以 `README.md`、`docs/FEATURES.md`、`docs/DEMO.md`、`docs/DEPLOYMENT.md` 和 `docs/SECURITY.md` 为准；历史设计、旧计划、台账和过程报告只在 `docs/archive/README.md` 导航。`REFLECTION.md` 只是学生本人填写的提纲，绝不代表学生个人反思或验收结论。
