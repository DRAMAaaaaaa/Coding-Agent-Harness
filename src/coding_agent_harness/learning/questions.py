...
from coding_agent_harness.learning.intent import IntentCard, IntentKind, IntentProjector, _clip
...
class QuestionService:
    ...
    async def ask(self, task_id: UUID, card_id: str, question: str) -> ReadOnlyAnswer:
        ...
        safe_question = _safe_question(self._redactor, question)
        ...
        try:
            ...
            request = LLMRequest(messages=[{
                "role": "user",
                "content": _question_context(card, events, self._redactor, safe_question),
            }])
            ...
            raise
        answer = _clip_answer(self._redactor, response.content)
...
def _safe_question(redactor: Redactor, value: str) -> str:
    sanitized = redactor.sanitize(value).value
    if not isinstance(sanitized, str) or sanitized != value:
        raise ValueError("SENSITIVE_VALUE_REJECTED")
    return value


def _clip_answer(redactor: Redactor, value: str) -> str:
    sanitized = redactor.sanitize(value).value
    return _clip_bytes(sanitized if isinstance(sanitized, str) else "", MAX_ANSWER_BYTES)


def _question_context(card: IntentCard, events: list[TaskEvent], redactor: Redactor, question: str) -> str:
    evidence = {event.sequence: event for event in events}
    summaries = [
        f"#{sequence} {evidence[sequence].event_type}: {_event_text(redactor, evidence[sequence])}"
        for sequence in card.evidence_sequences if sequence in evidence
    ]
    summary = _clip_bytes("\n".join(summaries), MAX_EVENT_SUMMARY_BYTES)
    return f"卡片:\n{card.actual_result}\n事件摘要:\n{summary}\n问题:\n{question}"


def _event_text(redactor: Redactor, event: TaskEvent) -> str:
    sanitized = redactor.sanitize(event.payload).value
    return _clip(str(sanitized))
