...
7. DeepSeek/Qwen Profile、会话型 API Key、固定端点适配、任务绑定与真实 Provider 运行时；当前 `main` 没有 WebUI 连接测试/主动清除入口，也没有真实外网任务验收，因此不得把这些入口或验收写成已完成。
8. 完全离线的 Scripted Mock、确定性单元测试、三机制演示和浏览器 E2E。
9. Docker、GitHub Actions、GitLab `unit-test` 以及阿里云 ECS 公网 IP Mock 演示配置。

未完成能力只放在“已知限制与后续方向”：生产级公网认证/HTTPS、Provider WebUI 连接测试/主动清除与真实外网任务验收、长期多条记忆、多 Agent 调度、任意依赖安装/网络/Git push 工具和最终视觉设计。文档不得把配置存在、离线 Stub 通过或待外部验收写成产品已经完成。

## 4. 精确文件处理

### 4.1 保留并重写/整理

- `README.md`：重写为当前产品首页，包含项目简介、特色、已完成功能、快速开始、演示、分发、目录结构、安全边界、已知限制、第三方组件与许可证和课程交付导航。
...
计划前的实现证据复核补充：当前 `main` 的 `RuntimeOrchestratorRouter` 和 `ProviderRegistry` 已把 DeepSeek/Qwen 会话 Profile 接入自研 Agent 主循环，内部 `probe()` 也已存在；但 `provider_routes.py` 与当前 WebUI 尚未暴露 Probe/主动清除。以上述当前分支事实替代早期计划文档中的预期状态，介绍文档必须明确区分“代码路径已接线”“真实外网尚未验收”和“WebUI 管理入口尚未合入”。
