...
export interface ProviderProfile { id: string; kind: "deepseek" | "qwen"; model: string; version: number; configured: boolean; }
...
export interface IntentCard { id: string; task_id: string; kind: "plan" | "first_edit" | "verification_failure" | "final_delivery"; intent: string; evidence_sequences: number[]; action: string; expected_result: string; actual_result: string; status: string; source_event_sequence: number; learning_card_id: string | null; }
export interface EventListener { onEvent: (event: TaskEvent) => void; onConnection: (state: ConnectionState) => void; onInvalidEvent?: () => void; }
export interface HarnessApi { connectProject(path: string): Promise<Workspace>; trustProject(workspace: Workspace): Promise<Workspace>; createTask(workspaceId: string, requirement: string, providerProfileId?: string): Promise<Task>; listProviders?: () => Promise<ProviderProfile[]>; createProvider?: (kind: ProviderProfile["kind"], model: string) => Promise<ProviderProfile>; setSessionCredential?: (profileId: string, apiKey: string) => Promise<ProviderProfile>; approvePlan(taskId: string): Promise<Task>; runTask(taskId: string): Promise<Task>; approveFinal(taskId: string): Promise<Task>; getTask(taskId: string): Promise<Task>; getIntentCards?: (taskId: string) => Promise<IntentCard[]>; askQuestion?: (taskId: string, cardId: string, question: string) => Promise<{ content: string }>; subscribeEvents(taskId: string, after: number, listener: EventListener): () => void; }
