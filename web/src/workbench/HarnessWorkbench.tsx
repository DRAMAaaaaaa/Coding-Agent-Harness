...
export function HarnessWorkbench({ api }: HarnessWorkbenchProps) {
  ...
  useEffect(() => { if (!workspace || !api.getLatestProjectLearning) return; void api.getLatestProjectLearning(workspace.id).then((card) => setProjectLearning(card ?? undefined)).catch(() => setProjectLearning(undefined)); }, [api, workspace]);
  ...
  const resetProjectContext = (): void => {
    setTask(undefined); setEvents([]); setCards([]); setAnswer(""); setComparison(undefined); setLearningText(undefined); setProjectLearning(undefined); lastSequence.current = 0; setStateUnknown(false); setProviderResetVersion((value) => value + 1); previousLatest.current = "PROJECT"; setSelectedStage("PROJECT");
  };
  ...
  const createTask = (form?: FormEvent): void => { form?.preventDefault(); if (!workspace) return; void run("task", async () => { try { const created = await api.createTask(workspace.id, requirement, providerId || undefined); setEvents([]); setCards([]); setAnswer(""); setComparison(undefined); setTask(created); setSelectedStage("PLAN"); setMessage("计划已生成，等待计划事件到达。"); } finally { setProviderResetVersion((value) => value + 1); } }); };
  ...
  const createCorrection = (card: IntentCard): void => { if (!task || !api.createCorrectionBranch || !api.getCorrectionComparison) return; void run("correction", async () => { const branch = await api.createCorrectionBranch!(task.id, card.source_event_sequence, correction); setComparison(await api.getCorrectionComparison!(branch.id)); if (branch.child_task_id) setTask(await api.getTask(branch.child_task_id)); }); };
  ...
  return <main className="workbench-shell" aria-busy={busy !== undefined}>
    ...
    {selectedStage === "REPLAY" && <ReplayStage cards={cards} comparison={comparison} question={question} answer={answer} correction={correction} busy={mutationDisabled} onQuestion={setQuestion} onAsk={ask} onCorrection={setCorrection} onCreateCorrection={createCorrection} />}
