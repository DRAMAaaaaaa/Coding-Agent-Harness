    ...
    try:
        ...
        for event_type, next_state in (
            ...
            await events.append(
                ...
                expected_sequence=len(await events.list_for_task(task.id)),
            )
            state = next_state
        manager = WorktreeManager(workspace, state_root)
        ...
        monkeypatch.setattr(
            WorktreeManager,
            "freeze",
            lambda _self, _task_id: (_ for _ in ()).throw(RuntimeError("freeze failed")),
        )
        ...
        assert (
            await (await router._for(task.id)).task(task.id)
        ).state is TaskState.WAITING_FINAL_REVIEW
...
@pytest.mark.asyncio
async def test_cleanup_skips_a_frozen_correction_parent(tmp_path: Path) -> None:
    root = tmp_path / "frozen-parent"
    root.mkdir()
    for arguments in (
        ("init", "-b", "main"),
        ("config", "user.name", "Harness Tests"),
    ...
    database = await Database.open(tmp_path / "state" / "db.sqlite3")
    try:
        workspace = Workspace(
            id=uuid4(),
            root=root,
        ...
        router._workspaces_by_task[task.id] = workspace
        router.cleanup()
        assert router._workspaces_by_task == {}
    finally:
        await database.close()
