...
- **复审 Minor 延期记录：** M1：`tests/api/test_providers.py` 中“拒绝 NUL 和超长 Key”的测试名实际仅覆盖超长 Key，缺少 NUL 拒绝的直接行为测试。M2：Provider mutation 虽复用 `SessionGuard`，但新增测试缺 Origin/session 拒绝，以及 Key 不进入事件、存储或异常响应的直接行为断言。M3：单进程 `ProviderBindingCoordinator._locks` 不回收没有 waiter 的 profile lock，持续创建 profile 可累积小对象。三项均是单进程本地 MVP Minor，不影响安全主路径或 Task 2 验收；依用户成本策略不在本 Task 返工，后续 Provider 管理/维护工作复核，Web 相关测试边界可同时参照 `DW-MVP-006`。Task 2 状态保持“返工完成，待复审”。
