...
### 2026-07-31 — IMPL-MVP-4-TASK-9-DEMO-E2E

- **范围与技能：** 在隔离 `codex/e2e-demo` worktree、基线 `843bf02` 实现 MVP-4 Task 9；完整读取 Task brief、`SPEC.md` 12/13、两份计划及现有 Agent/API/Web 契约，使用 `systematic-debugging`、`test-driven-development` 与 `verification-before-completion`。未联网、下载浏览器、安装依赖、调用真实 LLM、接触凭据、实现 Docker/CI/Task 10、merge 或 push。
...
### 2026-07-31 — REWORK-MVP-4-TASK-10-SPEC-I1
...
### 2026-08-10 — CI PowerShell 测试独立审查返工

- **规约审查：** 独立审查结论 Spec compliant Yes、Ready for quality review Yes，Critical / Important / Minor=`0/0/0`。
...
- **工作树与基线：** 在 `E:\Coding Agent Harness\.worktrees\public-ip-mock-demo` 的 `codex/public-ip-mock-demo` 分支执行，基线为 `836ce93`；未复制 `.env`、私钥或运行状态，未操作真实 ECS、未读取凭据、未 push。
- **技能与 TDD：** 使用 `test-driven-development` 和 `verification-before-completion`。先新增公网 Host/Origin、Compose 与 Nginx 交付契约；RED 命令 `E:\Coding Agent Harness\.venv\Scripts\python.exe -m pytest tests/demo/test_serve_cleanup.py tests/distribution/test_delivery_files.py -q` 结果为 9 failed、22 passed，失败原因准确为缺少 `_trusted_request_targets`、Compose 覆盖与 Nginx 配置。
...
### 2026-08-14 — 公网 IP Mock 演示最终审查返工

- **过程回填：** 实现前同一陌生审计员的 READY 结论及原阻塞关闭事实已回填到 `SPEC_PROCESS.md`，并引用受忽略报告 `.superpowers/sdd/public-ip-cold-start-report.md`；实施计划 Step 1—8 已标记完成，Step 9 仅标记静态门禁，未把 Docker daemon、Nginx 容器或 ECS 动态验收写为完成。既有技术提交为 `b19a4d3`、`aecdbe8`、`51b5dff`。
- **RED→GREEN：** 最终审查指出无 Docker CLI 的 GitLab 环境会被无条件 subprocess 阻塞、GitHub 未强制 Compose 渲染、Nginx 无基础安全头、down 未带必填变量、HTTP 风险说明不足。先扩展交付契约，RED 为 `3 failed, 17 passed`（缺少安全头、幂等 down/风险文档、GitHub Compose 作业）；Compose 渲染原行为在本机首次通过。GREEN 后测试为 `20 passed`：无 Docker CLI 以明确 pytest capability skip，不吞掉 Compose 失败；GitHub docker-build 以固定非秘密变量强制 `config --format json`；业务 Nginx 响应加入三项 `always` 安全头；文档下线命令携带变量并明确 HTTP/无认证风险。
- **完整验证与意外返工：** 首次完整门禁暴露与公网功能无关的既有不稳定测试：同一 `approved_at` 时间戳下 `latest_for_workspace()` 按 UUID 升序而非最后插入记录排序。根因明确后，以现有回归断言复现并将查询二级排序改为 `rowid DESC`；`tests/learning/test_cards.py` 为 `5 passed`。最终 `mingw32-make test` 退出 0：Python `840 passed, 15 skipped`、Vitest `28 passed`、Playwright `3 passed, 1 skipped`；Ruff、mypy、ESLint、TypeScript、Vite build、`mingw32-make demo` 三项 PASS、秘密扫描、`pip check`、Compose JSON 与 `git diff --check` 全部通过。
...
### 2026-08-14 — 共学回放式工作台前端优化设计

- **任务切换：** 用户选择 Subagent-Driven 执行文档整理后，在 worktree 创建和 subagent 派发前要求先讨论前端优化，并在计划完成后与文档任务一起实施。原文档计划因此暂停，没有发生部分实现。
- **技能与决策：** 使用 `brainstorming`，核对当前 `App.tsx`、样式、React 测试和 Playwright 主路径。用户逐项选择六阶段单页工作台、自动推进/历史回看、Mock 默认/Provider 高级设置、意图优先/技术细节折叠、专业教学浅色风格、仅前端重构和桌面答辩优先。
- **设计结果：** 阶段为项目接入、需求描述、计划审批、执行与验证、回放与纠正、交付与经验；把阶段推导和最新证据选择提取为纯函数，把现有大组件拆为工作台、阶段组件、Provider 抽屉和证据组件。
- **安全与范围：** 不新增后端 API，不提供无效 Probe/清除按钮；API Key 保持会话内并在成功、失败、关闭和切换项目时清空。此阶段只写规格和过程记录，没有修改前端实现、安装依赖、调用真实 Provider 或触碰用户环境目录。

### 2026-08-14 — 前端工作台与交付文档联合实施计划

- **技能与输入：** 用户批准书面前端规格后使用 `writing-plans`，合并前端设计与尚未执行的文档整理计划；重新核对 `types.ts`、现有 App 测试、真实 Playwright Mock 主路径和文档归档边界。
- **任务拆分：** 五个串行 Task 分别交付阶段/证据纯函数、组件化工作台与视觉、浏览器/窄屏验收、历史归档、最终介绍/反思提纲；文档只能在 UI 稳定后重写。
...
- **复审返工：** 首次返工的外部复审为 Critical / Important / Minor=`0/1/0`，确认“无效的较晚候选”仍会保留旧 verification/diff 与摘要。代码提交 `8b5a26b` 按严格替换处理：每个 `VERIFICATION_SUCCEEDED` 先淘汰旧 verification/summary，仅在诊断有效时重设；已识别为 `git_diff` 的完成事件先淘汰旧 diff/summary，仅在结果完整时重设，普通工具完成不影响 diff。验证候选 RED 为 `1 failed, 19 passed`、GREEN 为 `20 passed`；git diff 候选 RED 为 `1 failed, 20 passed`、GREEN 为 `21 passed`，并断言普通 `read_file` 完成仍保留 DELIVERY。新鲜聚焦 Vitest 为 `1 passed, 21 passed`，ESLint、TypeScript 与 `git diff --check` 均退出 0；等待再次独立规约符合性与代码质量复审。
...
- **固定项目路径追加：** 用户随后要求演示中必须给用户一个确定路径并由用户自己输入。实现改为公网常驻参数组合 `--max-seconds 0 --keep-alive --runtime-root /state` 时直接使用 `/state` 作为 runtime root，并在启动前清理旧 `/state/fixture` 与 `/state/state`，从而让 WebUI 的“项目路径”固定为 `/state/fixture`；README、DEMO、DEPLOYMENT 与交付契约已同步。本地短时演示仍保留 `session-*` 与 ready JSON。
